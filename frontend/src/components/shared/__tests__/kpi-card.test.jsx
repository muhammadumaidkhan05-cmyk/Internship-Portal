...
describe("<KpiCard />", () => {
  ...
  it("does not render description element when omitted", () => {
    const { container } = render(
      <KpiCard icon={TestIcon} value="42" label="Items" />,
    );
  ...
  it("renders the icon", () => {
    const { container } = render(
      <KpiCard icon={TestIcon} value="0" label="Test" />,
    );
  ...
  const tones = ["blue", "cyan", "red", "amber", "violet", "green"];
  for (const tone of tones) {
    it(`renders tone '${tone}' without crashing`, () => {
      expect(() =>
        render(<KpiCard icon={TestIcon} value="0" label="Test" tone={tone} />),
      ).not.toThrow();
    });
  }
});
