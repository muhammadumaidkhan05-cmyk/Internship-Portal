      ...
      <input
        ...
        onKeyDown={(e) => {
          if (e.key === "Enter" && query.trim()) {
            e.preventDefault();
            handleSelect(`/super-admin/users?search=${encodeURIComponent(query.trim())}`);
          }
        }}
                    ...
                    <ul className="mt-1 space-y-1">
                      {users.map(u => (
                        <li key={u.id}>
                          <button
                            type="button"
                            onClick={() => handleSelect(`/super-admin/users?search=${encodeURIComponent(query)}`)}
                    ...
                    <ul className="mt-1 space-y-1">
                      {programs.map(p => (
                        <li key={p.id}>
                          <button
                            type="button"
                            onClick={() => handleSelect(`/super-admin/programs?search=${encodeURIComponent(query)}`)}
                            className="flex w-full items-center justify-between rounded-lg px-2 py-1.5 text-left text-sm transition-colors hover:bg-[#F8FAFC]"
                          >
                            <div>
